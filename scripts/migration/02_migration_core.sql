...
-- 2. Encounters
CREATE TABLE IF NOT EXISTS nessie.healthcare.encounters
WITH (format = 'AVRO')
AS SELECT * FROM mysql.healthcare.encounters;

-- 3. Allergies (Handling Reserved Keywords and Nulls)
CREATE TABLE IF NOT EXISTS nessie.healthcare.allergies
WITH (format = 'AVRO')
AS SELECT
    "START" as start_date,
...
CREATE TABLE IF NOT EXISTS nessie.healthcare.observations
WITH (format = 'AVRO')
AS SELECT
